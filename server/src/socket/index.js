  ...
  io.on('connection', (socket) => {
    ...
    socket.on('join-room', async ({ code, name, isHost, hostToken }) => {
      ...
      try {
        ...
        if (!olympic) return socket.emit('error', { message: 'Olympic not found' });

        let participantAdded = false;

        // Dynamically add participant when a non-host joins
        if (!isHost && name && name.trim()) {
          const trimName = name.trim().slice(0, 30);
          const alreadyIn = olympic.participants.some((p) => p.name === trimName);
          if (!alreadyIn) {
        ...
        if (participantAdded) {
          ...
          io.to(upperCode).emit('room-update', { olympic: safeOlympic, leaderboard });
        } else {
    ...
    socket.on('submit-score', async ({ code, result, hostToken }) => {
      try {
        ...
        io.to(code.toUpperCase()).emit('room-update', { olympic: safe2, leaderboard: leaderboard2 });
      } catch (err) {
    ...
    socket.on('finish-olympic', async ({ code, hostToken }) => {
      try {
        ...
        io.to(code.toUpperCase()).emit('room-update', { olympic: safe3, leaderboard: leaderboard3 });
        io.to(code.toUpperCase()).emit('olympic-finished', { code: code.toUpperCase() });
      } catch (err) {
