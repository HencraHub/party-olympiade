...
export default function HostRoomPage() {
  ...
  const { olympic, leaderboard, updateFromRoomEvent, setConnected } = useOlympicStore();
  ...
  useEffect(() => {
    ...
    socket.emit('join-room', { code: code.toUpperCase(), name: 'Host', isHost: true, hostToken });
  ...
  function submitScore(result) {
    const socket = getSocket();
    socket.emit('submit-score', { code: code.toUpperCase(), result, hostToken });
    setScoreGame(null);
    setTab('board');
  }

  function finishOlympic() {
    if (!confirm('End the Olympic and reveal the winner? This cannot be undone.')) return;
    setFinishing(true);
    const socket = getSocket();
  ...
  if (!olympic) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-muted animate-pulse">Connecting to room {code}…</div>
      </div>
    );
  }
      ...
      <div className="min-h-screen px-4 py-10">
        <div className="max-w-lg mx-auto space-y-5 animate-slide-up">
          ...
          <GlassCard glow className="text-center py-8">
            <h1 className="font-black text-white text-2xl mb-1">{olympic.name}</h1>
            <p className="text-sm text-muted mb-5">Share this code — players go to the website and enter it</p>
            <div
          ...
          <GlassCard>
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-sm text-muted">{playerCount} / {maxPlayers}</span>
            </div>
            ...
            {playerCount === 0 ? (
              ...
              <div className="space-y-2">
                {olympic.participants.map((p, i) => (
                  <div key={p.name} className="flex items-center gap-3 py-1 animate-fade-in">
      ...
      <div className="max-w-2xl mx-auto space-y-4">
        ...
        <GlassCard className="flex items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h1 className="font-black text-white text-lg truncate">{olympic.name}</h1>
            </div>
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                onClick={() => {
                  navigator.clipboard.writeText(`${window.location.origin}/join/${code?.toUpperCase()}`);
                }}
              >
                📋 Copy invite link
              </button>
        ...
        <div className="glass rounded-xl px-4 py-3">
          <div className="flex justify-between text-xs text-muted mb-1.5">
            <span>Progress</span>
            <span>{scoredCount} / {totalGames} games scored</span>
          </div>
        ...
        <div className="flex gap-1 glass rounded-xl p-1">
          {[
            ...
            <button
              key={key}
              onClick={() => { setTab(key); if (key === 'score') setScoreGame(currentGame); }}
        ...
        {tab === 'game' && currentGame && (
          <div className="space-y-4">
            ...
            {/* Drinking rules */}
            {currentGame.addons?.drinkingGame?.enabled && currentGame.addons.drinkingGame.rules && (
              <GlassCard className="border-orange-500/30">
                <h3 className="text-sm font-semibold text-orange-400 mb-1">🍺 Drinking Rules</h3>
                <p className="text-sm text-white whitespace-pre-line">{currentGame.addons.drinkingGame.rules}</p>
              </GlassCard>
            )}
            ...
            <button
              ...
              onClick={() => { setScoreGame(currentGame); setTab('score'); }}
            >
          ...
          <GlassCard>
            ...
            <div className="mb-4">
              ...
              <select
                ...
                value={scoreGame?._id || ''}
                onChange={(e) => {
                  const g = olympic.games.find((gm) => String(gm._id) === e.target.value);
                  setScoreGame(g || null);
                }}
              >
                ...
                {olympic.games.map((g) => (
                  <option key={g._id} value={g._id}>
                    {g.icon} {g.title}
                    {olympic.results.find((r) => String(r.gameId) === String(g._id)) ? ' ✅' : ''}
                  </option>
                ))}
              </select>
            </div>
            ...
            {scoreGame ? (
              <ScoreEntry
                ...
                existingResult={olympic.results.find((r) => String(r.gameId) === String(scoreGame._id))}
                onSubmit={submitScore}
                onCancel={() => setScoreGame(null)}
              />
            ) : (
              <p className="text-muted text-sm text-center py-4">Select a game above.</p>
            )}
          </GlassCard>
        )}
        ...
        {tab === 'players' && (
          <GlassCard>
            <h2 className="font-bold text-white mb-4">Participants ({olympic.participants.length})</h2>
            <div className="space-y-2">
              {olympic.participants.map((p) => (
                <div key={p.name} className="flex items-center gap-3 py-2 border-b border-white/5">
                  {p.avatarBase64 ? (
                    <img src={p.avatarBase64} alt={p.name} className="w-10 h-10 rounded-full object-cover" />
                  ) : (
          ...
          <GlassCard>
            ...
            <div className="space-y-1">
              {olympic.games.map((g, i) => {
                const scored = !!olympic.results.find((r) => String(r.gameId) === String(g._id));
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-2 text-sm py-1.5 px-2 rounded-lg transition-colors text-left ${
                      isCur ? 'bg-purple-500/20 text-white' : 'text-muted hover:text-white hover:bg-white/5'
                    }`}
                    onClick={() => { setScoreGame(g); setTab('score'); }}
                  >
                    <span className="w-5 text-center">{i + 1}.</span>
                    <span>{g.icon}</span>
                    <span className="flex-1 truncate">{g.title}</span>
                    {scored && <span className="text-green-400 text-xs">✅</span>}
                    {isCur && <span className="text-yellow-400 text-xs">▶</span>}
                  </button>
                );
              })}
            </div>
