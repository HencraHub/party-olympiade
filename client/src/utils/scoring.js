  ...
  for (let idx = 0; idx < results.length; idx++) {
    ...
    if (isFFA) {
      ...
      for (const { participantName, place } of placements) {
        ...
        scores[participantName].base += getPointsForPlace(scoringMode, place, n) * multiplier;
      ...
      if (extraRules.comebackPenalty && prevLeader && prevLeader === lastPlaceName && scores[prevLeader]) {
        scores[prevLeader].bonus -= 2;
        scores[prevLeader].bonusLog.push({ game: game.title, reason: "Comeback Penalty", delta: -2 });
      }
      if (extraRules.lastPlaceBonus && prevLast && prevLast === winnerName && scores[prevLast]) {
