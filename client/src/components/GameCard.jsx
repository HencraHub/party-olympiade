export default function GameCard({ game, isCurrent = false, isScored = false, onClick }) {
    ...
    >
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-2">
          ...
          {isScored && (
            <span className="badge bg-green-500/20 text-green-400 shrink-0">✅ Scored</span>
          )}
          {isCurrent && !isScored && (
            <span className="badge bg-yellow-500/20 text-yellow-400 shrink-0">▶ Live</span>
          )}
        </div>
        ...
        <div className="flex flex-wrap gap-1 mt-2">
          {addons.drinkingGame?.enabled && (
            <span className="badge bg-orange-500/15 text-orange-400">🍺 Drinking</span>
          )}
          {addons.timeLimit > 0 && (
            <span className="badge bg-blue-500/15 text-blue-400">⏱ {addons.timeLimit}min</span>
          )}
          {addons.equipment && (
            <span className="badge bg-gray-500/15 text-gray-400">🛠 Equipment</span>
          )}
          {addons.handicap && (
